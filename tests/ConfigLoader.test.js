...
describe('ConfigLoader', () => {
    ...
    test('applies CLI overrides relative to cwd', () => {
        ...
        expect(config.entry).toBe(path.resolve(PROJECT_ROOT, 'examples/src/main.lua'));
        expect(config.output).toBe(path.resolve(PROJECT_ROOT, 'dist/cli.lua'));
        expect(config.sourceRoot).toBe(path.resolve(PROJECT_ROOT, 'examples/src'));
    });

    test('throws descriptive error for invalid configuration', () => {
        const configPath = path.join(tempDir, 'luapack.config.json');
        fs.writeFileSync(configPath, JSON.stringify({ output: './dist/out.lua' }, null, 2));

        expect(() => loadConfig({ config: configPath })).toThrow(/Invalid configuration/);
    });
});
