...
function loadExampleConfig() {
    return loadConfig({ config: path.join(PROJECT_ROOT, 'examples/luapack.config.json') });
}
...
describe('DependencyAnalyzer', () => {
    test('builds graph with module metadata', () => {
        ...
        const { graph, entryModule } = analyzer.buildDependencyGraph(config.entry);
        ...
        expect(moduleNames).toEqual(expect.arrayContaining(['app.greeter', 'vendor.version']));
        expect(moduleNames[moduleNames.length - 1]).toBe(entryModule.moduleName);
    });
